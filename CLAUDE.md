...
- **핵심 철학** — "변동성이 높을 때 파는 것"이 아니라 "변동성은 높은데 가격은 갈 곳이 없을 때(평균회귀 장세) 파는 것". 추세 폭주 구간은 감점이 아니라 차단
...
- **추세장 차단 필터** (score 0으로 차단, 사유에 "차단:" 접두) — 30일 등락 ±`TREND_BLOCK_30D`(8%) 초과, SMA20 이격 ±`TREND_BLOCK_EXT20`(5%) 초과, 5일 신고/신저가 갱신 중(`new_5d_high`/`new_5d_low`), 볼린저밴드 이탈(%B<0 또는 >1). 추세 동반 고변동성은 `_hv_bonus`에서 가점 축소 (정당한 변동성)
- **지표** — pandas 없이 리스트 기반으로 계산: RSI(14, Wilder), 볼린저 %B(20, 2σ), SMA20/50, SMA20 이격률, HV20(20일 연율화 역사변동성), HV 랭크(1년 백분위), 30일 등락, 52주 위치, 5일 신고/신저가 플래그
- **신호 점수** 0~100 — 풋매도·양매도 `SIGNAL_THRESHOLD = 55`, 콜매도는 `CALL_SIGNAL_THRESHOLD = 65` (원자재 급등 꼬리 리스크), 강신호 ≥ 75
- **수수료 반영** — `fee_points`가 왕복 비용 `ROUND_TRIP_COST_USD`($12/레그)를 종목별 계약 승수(UNIVERSE 7번째 값, $/pt)로 포인트 환산. `apply_costs`가 비용 차감 후 `MAX_FEE_RATIO`(프리미엄의 15%)·`MIN_NET_ANNUALIZED`(명목가 기준 연 4%) 미달 신호를 제외. 양매도는 원시 후보(put_raw/call_raw)로 합산 프리미엄을 만들고 수수료 2배 차감 — 레그 개별 필터로 걸러내지 않음
- **청산 기준 (익절+손절)** — 모든 신호 사유에 익절 `TAKE_PROFIT_RATIO`(프리미엄 50% 이하 축소 시 환매)·손절 `STOP_LOSS_MULT`(2배 도달 시 환매) 세트 표기. 만기 보유가 아니라 반등 시 프리미엄 회수가 원칙
- **행사가·프리미엄·델타는 블랙-76 모델 추정치** — 선물 옵션 실시간 체인이 무료 데이터로 없으므로 HV20을 IV 프록시로 사용. `_norm_cdf`는 `math.erf` 기반(scipy 미사용). ATM에서 바깥으로 이동하며 델타 상한 내에서 목표 델타(풋 0.25/콜 0.20, 상한 풋 0.30/콜 0.25)에 **가장 가까운** 행사가 선택 — 행사가 간격이 넓은 종목이 목표를 지나쳐 프리미엄이 급감하는 것 방지. 화면에 "추정치이니 주문 전 HTS 실제 호가 확인" 경고 명시
- `RISK_FREE_RATE = 0.04`
- CLI: `python options_signal_bot.py --generate options.html` / 로컬 서버는 인자 없이 실행 (포트 8081)

## 백테스트 (`backtest_sellput.py`)

"급락 풋매도 → 반등 시 익절 환매" 전략 검증용 CLI (배포 대상 아님, options_signal_bot에서 모델·유니버스 재사용):
- 진입: z-점수(20일) ≤ `PANIC_Z`(-2.0) + HV 랭크 ≥ `PANIC_HV_RANK`(70)
- 청산 우선순위: 손절(2배) → 익절(`TAKE_PROFIT_RATIO` 50% 회수) → 패닉 해소(z ≥ `RESOLVE_Z` -0.5) → 만기 임박(DTE ≤ 7) → 만기 내재가치 정산
- 마킹 IV는 `mark_iv`가 보유일수에 따라 1년 중앙값으로 지수 회귀(`IV_REVERT_DAYS`=10) — HV가 급반등에서도 높게 유지되는 왜곡 보정(IV crush 근사)
- 동일 진입의 "만기 보유" 손익을 함께 계산해 청산 규칙 효과를 비교 출력. 포지션 슬롯은 규칙 청산 후에도 만기까지 점유(두 변형의 진입 동일 유지)
- 사용자의 외부 백테스트 도구(Back Test Lab, PANIC_FADE_FOP_V1)가 익절 규칙 없이 만기 보유로 손실 낸 사례를 계기로 작성 — 오프라인 테스트는 `scratchpad/test_backtest.py`
