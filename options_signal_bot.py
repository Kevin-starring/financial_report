...
- 리스크 관리: 모든 신호에 익절(프리미엄 50% 회수 시 환매)·손절(2배 도달 시 환매) 기준 표기
...
# 리스크 관리 — 신호 카드에 청산 계획(익절+손절)으로 표기
STOP_LOSS_MULT = 2.0           # 옵션 가격이 수취 프리미엄의 2배 도달 시 손절 환매 권고
TAKE_PROFIT_RATIO = 0.5        # 옵션 가격이 수취 프리미엄의 50% 이하로 축소 시 익절 환매 권고
...
def _make_signal(strategy, score, reasons, candidate, today):
    ...
    reasons.append(
        f"⚠ 청산 계획 — 익절: 옵션 가격이 수취 프리미엄의 {TAKE_PROFIT_RATIO*100:.0f}%"
        f"({candidate['premium']*TAKE_PROFIT_RATIO:.4g} pt) 이하로 축소 시 환매 청산 (반등으로 프리미엄이 "
        f"줄었을 때 이익 확정, 만기까지 보유하지 않음) / 손절: {STOP_LOSS_MULT:.0f}배"
        f"({candidate['premium']*STOP_LOSS_MULT:.4g} pt) 도달 시 환매 청산 — 추세 전환 시 손실 제한"
    )
    return {
...
<div class="cards">
  <div class="sig-card">
    <div class="reasons">
      ...
      <div class="reason"><strong>청산 기준 (익절+손절)</strong> — 모든 신호에 익절(옵션 가격이 수취 프리미엄의 50% 이하로 축소 시 환매)과 손절(2배 도달 시 환매) 기준을 함께 제시. 옵션 매도는 만기까지 버티는 전략이 아니라, 기초자산 반등으로 프리미엄이 줄었을 때 회수하는 전략 — 익절 없이 보유하면 가격이 되돌아올 때 이익을 반납. 손실은 이론상 무한대이므로 기계적 손절도 필수.</div>
